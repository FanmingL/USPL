...
tuplenames = ('state', 'priv_state', 'last_state', 'last_priv_state', 'last_action', 'action', 'expert_action', 'next_state',
              'next_priv_state', 'reward', 'logp', 'mask', 'start', 'done', 'reward_input', 'timeout', 'target_embedding', 'embedding_noise', 'target_logstd', 'target_flag', 'next_target_logstd')
...
class MemoryArray(object):
    def __init__(self, max_transition_num: int=1000000, max_traj_step: Optional[int]=1000, rnn_slice_length=1):
        ...
        self.multi_memory: Dict[int, List[Transition]] = {}
    ...
    def traj_push(self, memory_array: np.ndarray):
        if self.memory_buffer is None:
            assert False
        traj_len = memory_array.shape[0]
        # clear trajectory_start, trajectory_length, transition_count, transition_buffer
        remove_traj_num = 0
        if self.transition_count + traj_len > self.max_transition_num:
            transition_count = self.transition_count
        ...
        self.trajectory_start.append(self.ptr)
        self.memory_buffer[self.ptr:self.ptr+traj_len, :] = memory_array.copy()
        self.ptr += traj_len
        # for ind, transition in enumerate(memory):
        #     self.memory_buffer[self.ptr] = 0
        #     self._insert_transition(transition)
        #     self.ptr += 1
        self.trajectory_length.append(traj_len)
        self.transition_count += traj_len
        if self.ptr >= self.max_transition_num:
            self.ptr = 0
    ...
    def mem_push_idx(self, transition: Transition, idx, max_episode_length, *args):
        if idx in self.multi_memory:
            memory = self.multi_memory[idx]
        else:
            self.multi_memory[idx] = []
            memory = self.multi_memory[idx]

        memory.append(transition)
        done = transition.done if np.isscalar(transition.done) else np.array(transition.done)
        mask = transition.mask if np.isscalar(transition.mask) else np.array(transition.mask)
        timeout = transition.timeout if np.isscalar(transition.timeout) else np.array(transition.timeout)
        if done:
            if len(memory) > max_episode_length:
                for i, item in enumerate(memory):
                    print('step validation', i+1, item.done, item.timeout, item.logp)
                print(idx, *args)
            if mask:
                if timeout:
                    memory = memory[:max_episode_length-1]
                    memory[-1].done[:] = 1
                    memory[-1].timeout[:] = 1
                    self.complete_traj(memory)
                else:
                    self.complete_traj(memory)
            self.multi_memory.pop(idx)
