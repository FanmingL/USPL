...
class RNNHidden:
    ...
    def set_rnn_start(self, rnn_start):
        self._rnn_start = rnn_start.to(self._device)

    def set_attention_concat_mask(self, attention_concat_mask):
        self._attention_concat_mask = attention_concat_mask.to(self._device)
        ...
        for i in range(self.size):
            ...
            if cur_data is not None:
                if isinstance(cur_data, tuple):
                    ...
                    cur_data.max_batch_size -= 1
                    for k in cur_data.key_value_memory_dict:
                        cur_data.key_value_memory_dict[k] = cur_data.key_value_memory_dict[k][1:]
                    if cur_data.lengths_per_sample is not None:
                        cur_data.lengths_per_sample = cur_data.lengths_per_sample[1:]
                else:
    ...
    def hidden_batch_size(self) -> int:
        ...
        if self._data[0] is not None:
            if isinstance(self._data[0], tuple):
                ...
                return self._data[0].max_batch_size
            else:
    ...
    def copy_attachment_from_(self, other: "RNNHidden"):
        if other._rnn_start is not None:
            self._rnn_start = other._rnn_start.clone()
        if other._attention_concat_mask is not None:
            self._attention_concat_mask = other._attention_concat_mask.clone()
        if other._mask is not None:
            self._mask = other._mask.clone()
        if other._grad_detach is not None:
            self._grad_detach = other._grad_detach.clone()
    ...
    def __deepcopy__(self, memo: Dict) -> "RNNHidden":
        ...
        for item in self._data:
            if isinstance(item, tuple):
                ...
                new_hidden = InferenceParams(item.max_seqlen, item.max_batch_size)
                new_hidden.seqlen_offset = item.seqlen_offset
                new_hidden.batch_size_offset = item.batch_size_offset
                if item.lengths_per_sample is not None:
                    new_hidden.lengths_per_sample = item.lengths_per_sample.clone()
                for k in item.key_value_memory_dict:
                    new_hidden.key_value_memory_dict[k] = item.key_value_memory_dict[k].clone()
                data_list.append(new_hidden)
            else:
    ...
    def to_device(self, device: torch.device) -> None:
        if not self._device == device:
            ...
            self._mask = self._mask.to(device) if self._mask is not None else None
            self._rnn_start = self._rnn_start.to(device) if self._rnn_start is not None else None
            self._attention_concat_mask = self._attention_concat_mask.to(device) if self._attention_concat_mask is not None else None
    ...
    def hidden_state_mask_reset_(self, masks: Union[np.ndarray, torch.LongTensor]) -> None:
        assert self._batch_first == False, f'only when batch_first is False, this function is callable'
        for i in range(self.size):
            if isinstance(self._data[i], tuple):
                self._data[i][0].squeeze(0)[masks] = 0
                self._data[i][1].squeeze(0)[masks] = 0
            elif InferenceParams is not None and isinstance(self._data[i], InferenceParams):
                for k in self._data[i].key_value_memory_dict:
                    self._data[i].key_value_memory_dict[k][masks] = 0
                if self._data[i].lengths_per_sample is not None:
                    self._data[i].lengths_per_sample[masks] = 0
            else:
                self._data[i].squeeze(0)[masks] = 0
    ...
    def hidden_state_sample_(self, idxes: Union[List[int], np.ndarray]) -> None:
        ...
        for i in range(self.size):
            ...
            if isinstance(self._data[i], tuple):
                ...
                for k in cur_data.key_value_memory_dict:
                    cur_data.key_value_memory_dict[k] = cur_data.key_value_memory_dict[k][idxes]
                    cur_data.max_batch_size = cur_data.key_value_memory_dict[k].shape[0]
                if cur_data.lengths_per_sample is not None:
                    cur_data.lengths_per_sample = cur_data.lengths_per_sample[idxes]
            else:
                self._data[i] = self._data[i][:, idxes]
        self._rnn_start = self._rnn_start[idxes, ...] if self._rnn_start is not None else None
        self._mask = self._mask[idxes, ...] if self._mask is not None else None
        self._attention_concat_mask = self._attention_concat_mask[idxes, ...] if self._attention_concat_mask is not None else None
        # if self._attention_concat_mask is not None:
        #     _attention_concat_mask_max = self._attention_concat_mask.max(dim=0, keepdim=True).values
        #     zero_positions = (_attention_concat_mask_max == 0).nonzero(as_tuple=False)
        #     if zero_positions.numel() > 0:
        #         first_zero_pos = zero_positions[0, 1].item()
        #         self._attention_concat_mask = self._attention_concat_mask[:, :first_zero_pos]
    ...
    def hidden_detach_(self) -> None:
        ...
        for i in range(self.size):
            ...
            if isinstance(self._data[i], tuple):
                ...
                for k in cur_data.key_value_memory_dict:
                    cur_data.key_value_memory_dict[k] = cur_data.key_value_memory_dict[k].detach()
                if cur_data.lengths_per_sample is not None:
                    cur_data.lengths_per_sample = cur_data.lengths_per_sample.detach()
            else:
    ...
    def hidden_state_slice_(self, start: int, end: int) -> None:
        ...
        for i in range(self.size):
            ...
            if isinstance(self._data[i], tuple):
                ...
                for k in cur_data.key_value_memory_dict:
                    cur_data.key_value_memory_dict[k] = cur_data.key_value_memory_dict[k][start: end]
                    cur_data.max_batch_size = cur_data.key_value_memory_dict[k].shape[0]
                if cur_data.lengths_per_sample is not None:
                    cur_data.lengths_per_sample = cur_data.lengths_per_sample[start: end]
            else:
