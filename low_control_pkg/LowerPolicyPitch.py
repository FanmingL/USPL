...
class LowerPitchPolicy:
    def __init__(self, ):
        self.policy_args = {'state_dim': 173,
                            'action_dim': 12,
                            'priv_state_dim': 205,
                            'embedding_size': 128,
        ...
        self.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'pt_pitch'), map_location=torch.device('cpu'))
        self.last_action_input = None
        self.last_state_input = None
        self.last_reward_input = None

        self.target_yaw_rad = None
        self.target_pitch_rad = None
        self.target_velocity = None
        self.step_cnt = None
        self.rnn_hidden = None
        self.target_update_interval = 15 # 10 for default
        self.reset()
    ...
    def reset(self, reset_buf=None):
        if reset_buf is None:
            ...
            if self.target_yaw_rad is not None:
                self.target_yaw_rad[reset_buf] = -100
            if self.target_pitch_rad is not None:
                self.target_pitch_rad[reset_buf] = 0.0
            if self.target_velocity is not None:
                self.target_velocity[reset_buf] = 0.6
            if self.step_cnt is not None:
                self.step_cnt[reset_buf] = 0

    def update_target(self, target_yaw_rad: torch.Tensor, forward_cmd: torch.Tensor, pitch_cmd: torch.Tensor):
        if self.step_cnt is None:
            self.step_cnt = 0 * target_yaw_rad
        step_mask = ((self.step_cnt % self.target_update_interval)==0)
        if self.target_yaw_rad is None:
            self.target_yaw_rad = target_yaw_rad.clone()
        else:
            mask = self.target_yaw_rad < -20
            self.target_yaw_rad[mask] = target_yaw_rad[mask]
            self.target_yaw_rad[step_mask] = target_yaw_rad[step_mask].clone()

        if self.target_pitch_rad is None:
            self.target_pitch_rad = pitch_cmd.clone()
        else:
            self.target_pitch_rad[step_mask] = pitch_cmd[step_mask].clone()
        if self.target_velocity is None:
            self.target_velocity = forward_cmd.clone()
        else:
            self.target_velocity[step_mask] = forward_cmd[step_mask].clone()

        self.step_cnt += 1


    def get_obs(self, env, target_yaw_rad: torch.Tensor, forward_cmd: torch.Tensor, pitch_cmd: torch.Tensor) -> torch.Tensor:
        ...
        assert len(pitch_cmd.shape) == 2
        assert pitch_cmd.shape[-1] == 1
        ...
        ego_pitch_cmd = pitch_cmd - env.pitch.reshape((-1, 1))
    ...
    @torch.no_grad()
    def forward(self, env, target_yaw_rad: torch.Tensor, forward_cmd: torch.Tensor, pitch_cmd: torch.Tensor) -> torch.Tensor:
        self.update_target(target_yaw_rad, forward_cmd, pitch_cmd)
        obs = self.get_obs(env, self.target_yaw_rad, self.target_velocity, self.target_pitch_rad)
